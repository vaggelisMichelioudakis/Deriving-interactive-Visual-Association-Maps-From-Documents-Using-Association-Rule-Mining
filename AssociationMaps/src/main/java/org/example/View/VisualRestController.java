...
public class VisualRestController {
    ...
    public static String readPdfContent(File pdfFile) throws IOException {
        ...
        try (PDDocument document = PDDocument.load(pdfFile)) {
            // Create the text stripper
            PDFTextStripper pdfStripper = new PDFTextStripper();

            // CRITICAL: Force the extractor to organize text based on
            // X,Y coordinates rather than the internal file structure.
            // This fixes most multi-column and form layout issues.
    ...
    public ResponseEntity<GraphDt> EnterValues(@RequestBody ValueRequest request) {
        ...
        try {
            ...
            if (file.exists() && file.isDirectory()) {
                ...
                for (File fileEntry : file.listFiles()) {
                    ...
                    try {
                        ...
                        if (fileName.endsWith(".txt")) {
                            // Use your existing method for TXT
                            content = readFileAsString(fileEntry.getAbsolutePath());
                        } else if (fileName.endsWith(".pdf")) {
                            // Use PDFBox for PDF
                            content = readPdfContent(fileEntry);
                        } else {
                            // Skip files that aren't txt or pdf
                            System.out.println("Skipping unsupported file: " + fileName);
                            continue;
                        }

                        // If content is empty/null, skip processing
                        ...
                        i++; // Only increment transaction ID if we actually processed a file
                ...
                System.err.println("Directory does not exist or is wrong path: " + file.getAbsolutePath());
            }
